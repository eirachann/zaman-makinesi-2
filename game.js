...
// Bağlantı durumu: her istemci belirli aralıklarla kendi "lastSeen" zamanını günceller.
// Diğer oyuncular bu zamana bakarak yeşil/kırmızı nokta gösterir.
const HEARTBEAT_MS = 8000;
const CEVRIMICI_ESIGI_MS = 15000;

async function kalpAtisiGonder() {
    if (!sonVeri || !sonVeri.players) return;
    const guncelOyuncular = sonVeri.players.map((p) =>
        p.id === playerId ? { ...p, lastSeen: Date.now() } : p
    );
    try {
        await updateDoc(odaRef, { players: guncelOyuncular });
    } catch (err) {
        console.error("Kalp atışı gönderilemedi:", err);
    }
}
...
function tumDonemleriGetir() {
    const ozelDonemler = sonVeri?.customEras || [];
    return [...donemler, ...ozelDonemler];
}
...
function cevrimiciMi(p) {
    return (Date.now() - (p.lastSeen || 0)) < CEVRIMICI_ESIGI_MS;
}
        ...
        <div class="player-group">
            ...
            ${aktifler.map(p => `
                <div class="player-card ${p.id === playerId ? 'is-me' : ''}">
                    <span><span class="presence-dot ${cevrimiciMi(p) ? 'online' : ''}"></span> ${p.isHost ? '👑 ' : ''}${p.name}</span>
                    ${atmaBtnHTML(p)}
                </div>
            `).join('')}
        </div>
    ...
    if (bekleyenler.length > 0) {
        html += `
            <div class="player-group" style="margin-top: 15px;">
                ...
                ${bekleyenler.map(p => `
                    <div class="player-card ${p.id === playerId ? 'is-me' : ''}">
                        <span><span class="presence-dot ${cevrimiciMi(p) ? 'online' : ''}"></span> ${p.name}</span>
...
function kartEklemeModaliniAc() {
    ...
    document.getElementById("kartKaydetBtn").onclick = async () => {
        ...
        const mevcutOzel = sonVeri.customEras || [];
        await updateDoc(odaRef, {
            customEras: [...mevcutOzel, yeniKart]
        });

        alert("Özel kart başarıyla eklendi!");
        closeModal();
        ekraniCiz();
    };
}
...
// "Geçmiş Günler Gelecek" modu: yolcu hariç herkese BAĞIMSIZ rastgele bir dönem/evren atanır.
// Bu modda "doğru cevap" yoktur; oyuncular yolcunun günlük hayatına kendi dönemlerinden tepki verir.
async function gecmisGelecekBaslat() {
    const tumSecenekler = tumDonemleriGetir();
    if (tumSecenekler.length === 0) return;

    const digerOyuncular = (sonVeri.players || []).filter(
        (o) => !o.isHost && o.id !== sonVeri.round.traveler && o.status !== "spectator" && o.status !== "waiting"
    );

    const perPlayerEras = {};
    digerOyuncular.forEach((o) => {
        const rastgele = tumSecenekler[Math.floor(Math.random() * tumSecenekler.length)];
        perPlayerEras[o.id] = rastgele.id;
    });

    await updateDoc(odaRef, {
        isLobbyMode: false,
        "round.perPlayerEras": perPlayerEras,
        "round.status": "oynaniyor",
        "round.timerEndsAt": Date.now() + SURE_SANIYE * 1000,
        "round.timerRemaining": SURE_SANIYE,
        "round.paused": false
...
function ekraniCiz() {
    ...
    if (round.status === "donem_bekleniyor") {
        ...
        if (benHost) {
            ...
            if (oyunModu === "gecmis_gelecek") {
                donemSecimHTML = `
                    <p style="font-size:0.85rem; color:#cbd5e1; margin-bottom:5px;">
                        🔮 Bu modda dönem seçmene gerek yok — turu başlattığında yolcu hariç herkese
                        birbirinden BAĞIMSIZ, rastgele bir dönem/evren atanacak.
                    </p>
                `;
            } else {
                donemSecimHTML = `
            ...
            let sabotajciHTML = "";
            if (sonVeri.settings?.sabotajci && oyunModu !== "gecmis_gelecek") {
            ...
            html += `<div class="host-panel">
                        ...
                        <div class="hostButonlar" style="margin-top:10px;">
                            <button id="turuBaslatBtn">${oyunModu === "gecmis_gelecek" ? "🎲 Herkese Farklı Dönem Ata ve Başlat" : "▶️ Turu Başlat"}</button>
    ...
    if (round.status === "oynaniyor" || round.status === "duraklatildi" || round.status === "sonuc") {
        if (oyunModu === "gecmis_gelecek") {
            if (yolcuMu) {
                html += `<div class="rol-karti yolcu">
                            ...
                            <p>Bu modda tahmin etmen gereken bir şey yok. Günlük/modern hayatından
                            sıradan şeyler anlat (dün ne yaptın, ne yiyorsun, telefonunla ne yapıyorsun vs).
                            Diğerleri kendi farklı zamanlarından gelmiş gibi tepki verecek — sen de onların
                            tepkilerinden eğlenmeye bak!</p>
                         </div>`;
            } else {
                const kendiDonemId = round.perPlayerEras?.[playerId];
                const kendiDonem = kendiDonemId ? donemBul(kendiDonemId) : null;

                html += `
                <div class="bilgilendirme" style="margin-bottom: 12px;">
                    <p>🔮 Bu senin GİZLİ, kişisel dönemin — diğer oyuncular başka dönemlerden gelmiş olabilir.
                    Yolcunun anlattığı modern şeylere KENDİ döneminden biriymişsin gibi tepki ver.</p>
                </div>
                <div class="bilgi-karti">
                    <h2>📜 ${kendiDonem ? kendiDonem.isim : "Dönemin bekleniyor..."}</h2>
                    ${kendiDonem ? `
                    <div class="kartBolum">
                        <h3>🔑 Anahtar Kelimeler</h3>
                        <div class="etiketler">${kendiDonem.anahtarKelimeler.map((k) => `<span class="etiket">${k}</span>`).join("")}</div>
                    </div>
                    <div class="kartBolum">
                        <h3>💬 Örnek Cümleler</h3>
                        <ul>${kendiDonem.ornekCumleler.map((c) => `<li>${c}</li>`).join("")}</ul>
                    </div>
                    <div class="kartBolum">
                        <h3>🚫 Yasaklı Kelimeler</h3>
                        <div class="yasakKutusu">${kendiDonem.yasakliKelimeler.map((k) => `<span class="yasakEtiket">${k}</span>`).join("")}</div>
                    </div>` : ""}
                </div>`;
            }
        } else if (yolcuMu) {
        ...
        if (round.status === "sonuc") {
            ...
            if (oyunModu === "gecmis_gelecek") {
                html += `<button id="turuBitirBtn">🎬 Turu Bitir</button>`;
            } else {
        ...
        if (benHost) {
            ...
            const turuBitirBtn = document.getElementById("turuBitirBtn");
            if (turuBitirBtn) turuBitirBtn.onclick = () => turuSonuclandir("tamamlandi");
...
// Bağlantı durumunu (yeşil/kırmızı nokta) canlı tutmak için düzenli kalp atışı
setTimeout(kalpAtisiGonder, 1000);
setInterval(kalpAtisiGonder, HEARTBEAT_MS);