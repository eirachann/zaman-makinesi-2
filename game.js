...
function ekraniCiz() {
    ...
    const benIzleyiciyimMi = !!(benimVerim && benimVerim.status === "spectator");
    ...
    if (benIzleyiciyimMi) {
        html += `<p style="text-align:center; color:#94a3b8; font-size:0.85rem; margin-bottom:10px;">👁️ İzleyici modundasın — sadece izliyorsun, oynayamazsın.</p>`;
    }
    ...
    if (round.status === "oynaniyor" || round.status === "duraklatildi" || round.status === "sonuc") {
        ...
        } else if (benIzleyiciyimMi) {
            // İzleyici hiçbir detayı (anahtar kelime, örnek cümle, yasaklı kelime, sabotajcı kimliği) görmemeli — sadece dönem/evren adı(ları)
            let izleyiciBaslik = donem ? donem.isim : "Dönem seçiliyor...";
            if (ikinciDonem) {
                izleyiciBaslik += ` + ${ikinciDonem.isim} (Kırık Zaman)`;
            }

            html += `
            <div class="bilgi-karti" style="text-align:center;">
                <h2>📜 ${izleyiciBaslik}</h2>
                <p style="color:#94a3b8; font-size:0.85rem; margin-top:8px;">
                    👁️ İzleyici olarak sadece dönem/evren adını görebilirsin — anahtar kelimeler,
                    örnek cümleler, yasaklı kelimeler ve varsa sabotajcının kimliği sana gösterilmiyor.
                </p>
            </div>`;
        } else {
            const benSabotajciyimMi = round.sabotajciId === playerId;

            if (benSabotajciyimMi) {
                // Sabotajcı gerçek dönemi GÖRMEMELİ — sadece kendisine verilen sahte rolü görür
                ...
                <div style="background: rgba(239, 68, 68, 0.2); border: 1px solid rgba(239, 68, 68, 0.5); padding: 15px; border-radius: 10px; margin-bottom: 15px;">
                    ...
                    <p>Gerçek dönemi/evreni SEN de bilmiyorsun — göreviniz aşağıdaki SAHTE rolü çaktırmadan
                    oynayıp herkesin kafasını karıştırmak.</p>
                </div>
                <div class="bilgi-karti">
                    <h2>📜 ${sabDonem ? sabDonem.isim : "Bilinmiyor"} <span style="font-size:0.7rem; color:#fca5a5;">(gizli/sahte rolün)</span></h2>
                    ${sabDonem ? `
                    <div class="kartBolum">
                        <h3>🔑 Anahtar Kelimeler</h3>
                        <div class="etiketler">${sabDonem.anahtarKelimeler.map((k) => `<span class="etiket">${k}</span>`).join("")}</div>
                    </div>
                    <div class="kartBolum">
                        <h3>💬 Örnek Cümleler</h3>
                        <ul>${sabDonem.ornekCumleler.map((c) => `<li>${c}</li>`).join("")}</ul>
                    </div>
                    <div class="kartBolum">
                        <h3>🚫 Yasaklı Kelimeler</h3>
                        <div class="yasakKutusu">${sabDonem.yasakliKelimeler.map((k) => `<span class="yasakEtiket">${k}</span>`).join("")}</div>
                    </div>` : ""}
                </div>`;
            } else {
        ...
        if (round.status === "sonuc") {
            ...
            let sabotajciSonuc = "";
            if (sonVeri.settings?.sabotajci && round.sabotajciId && !benIzleyiciyimMi) {
